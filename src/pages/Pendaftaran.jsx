  ...
  return (
    <Page>
      <div className="space-y-8">
        <div>
          <h2 className="text-2xl font-bold text-slate-800">Perekaman Pasien Baru</h2>
          <p className="text-slate-500">Lengkapi data pasien sebelum melakukan identifikasi biometrik.</p>
        </div>

        <main>
          <div className={`bg-white rounded-3xl shadow-sm border border-slate-100 p-8 grid grid-cols-1 md:grid-cols-2 gap-8 ${fadeIn ? "opacity-100" : "opacity-90"}`}>
            
            {/* NO RM */}
            <div className="flex flex-col gap-2">
              <label className="text-sm font-bold text-slate-700 ml-1">No. Rekam Medis</label>
              <input value={form.noRM} disabled className="rounded-xl border px-5 py-3.5 bg-slate-100" />
            </div>
            ...
            <div className="flex flex-col gap-2">
              ...
              <input type="date" name="tanggalLahir" value={form.tanggalLahir} onChange={handleChange} className="rounded-xl border px-5 py-3.5" />
            </div>

            {/* UMUR */}
            <Input label="Umur" name="umur" value={form.umur} disabled />

            {dropdown("jenisKelamin", "Jenis Kelamin", ["Laki-laki", "Perempuan"])}
...
/>
            ...
            {dropdown("provinsi", "Provinsi", ["Jawa Barat", "DKI Jakarta", "Jawa Tengah"])}
            {dropdown("kota", "Kota/Kabupaten", ["Bandung", "Jakarta", "Semarang"])}
            {dropdown("kecamatan", "Kecamatan", ["Coblong", "Cicendo", "Lengkong"])}
            <Input
  label="No. Telepon"
  name="telepon"
  type="number"
  ...
  onChange={handleChange}
/>
            {dropdown("agama", "Agama", ["Islam", "Kristen", "Katolik", "Hindu", "Buddha", "Konghucu"])}
            {dropdown("statusPerkawinan", "Status Perkawinan", ["Belum Kawin", "Kawin", "Cerai"])}
            {dropdown("pekerjaan", "Pekerjaan", ["PNS", "Swasta", "Wiraswasta", "Pelajar"])}
            {dropdown("pendidikan", "Pendidikan", ["SD", "SMP", "SMA", "D3", "S1", "S2"])}
            <Input
  label="Nama Ibu"
  name="namaIbu"
  value={form.namaIbu}
...
/>
            ...
            {dropdown("pekerjaanIbu", "Pekerjaan Ibu", ["IRT", "Swasta", "PNS"])}
            <Input
  label="Nama Ayah"
  name="namaAyah"
  value={form.namaAyah}
...
/>
            ...
            {dropdown("pekerjaanAyah", "Pekerjaan Ayah", ["Swasta", "PNS", "Wiraswasta"])}
            <Input
...
/>
            ...
            <div className="md:col-span-2 flex flex-col gap-2">
              ...
              <textarea name="catatan" value={form.catatan} onChange={handleChange} className="rounded-xl border px-5 py-3.5" />
            </div>
        ...
        {loading ? (
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
              ...
              <motion.div
                whileHover={{ y: -6 }}
                className="border border-slate-200 rounded-3xl p-8 flex flex-col items-center gap-6 hover:border-blue-300 transition-all group bg-gradient-to-br from-white to-slate-50"
              >
                <div className="w-full h-44 bg-white rounded-2xl flex items-center justify-center relative overflow-hidden shadow-inner">
                  <ScanFace className="w-20 h-20 text-slate-400 group-hover:text-blue-500 transition-colors" />
                  <div className="absolute left-0 right-0 h-[3px] bg-blue-500 shadow-[0_0_15px_rgba(59,130,246,0.8)] animate-scan" />
                </div>
    ...
    >
      ...
      >
        ...
        <div className="mb-6 text-center">
          {fingerVerified && (
            <p className="text-green-600 font-semibold">✔ Sidik Jari Terverifikasi</p>
          )}
          {faceVerified && (
            <p className="text-green-600 font-semibold">✔ Wajah Terverifikasi</p>
          )}
        </div>
...
function Input({ label, name, type = "text", disabled = false, value, onChange }) {
  return (
    <div className="flex flex-col gap-2">
      <label className="text-sm font-bold text-slate-700 ml-1">{label}</label>
      <input
